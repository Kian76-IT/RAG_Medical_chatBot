...
def precision_at_k(retrieved, relevant, k):
    ...
    relevant_count = sum([1 for doc in retrieved_k if doc == relevant])
...
for model_name, model_path in MODELS.items():
    ...
    # LOAD MODEL
    embedding_model = BaseEmbeddingModel(model_path)
    ...
    for query, relevant_doc in zip(queries,ground_truth):

        # QUERY EMBEDDING
        query_vec = embedding_model.encode([query])

        # RETRIEVE
        indices = retriever.search(query_vec,k=3)[0]

        # EVALUATION
        precision_scores.append(
            precision_at_k(indices,relevant_doc,3)
        )

        recall_scores.append(
            recall_at_k(indices,relevant_doc,3)
        )

        mrr_scores.append(
            reciprocal_rank(indices,relevant_doc)
        )

        hit_scores.append(
            hit_rate(indices,relevant_doc)
        )
...
# SAVE CSV
results_df.to_csv("evaluation/retrieval_results.csv",index=False)
