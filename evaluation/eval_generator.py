...
embedding_model = BaseEmbeddingModel("sentence-transformers/paraphrase-albert-small-v2")
...
# LIMIT EVALUATION
EVAL_SIZE = 100
eval_df = df.sample(EVAL_SIZE,random_state=42)
...
for idx, row in eval_df.iterrows():
    ...
    # RAG RETRIEVAL
    results = run_rag(query,embedding_model,retriever,df,k=3)
    context = "\n".join(results["context"].tolist())

    # GENERATE RESPONSE
    prediction = llm.generate(query,context)
...
results_df.to_csv("evaluation/generator_results.csv",index=False)
print(
